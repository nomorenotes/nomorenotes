class FSDHFs {
    ...
    async writeFile(path, data, { flags = "w" }) {
        ...
        const writable = await handle.createWritable({ keepExistingData: flags.includes("a") });
        writable.write(data);
        writable.close();
    }
    async unlink(path) {
    ...
    static resolvePath(path) {
        return ary_apply((typeof path === "string" ? path.split('/') : path), (value, i, ary) => (value === ".." ? (ary[i] = '', ary[i - 1] = '') : value === '.' ? ary[i] = '' : value)).filter(part => part);
    }
    async resolveDir(path, options) {
        const parts = FSDHFs.resolvePath(path);
        return parts.reduce(async (handle, name) => (await handle).getDirectoryHandle(name, options), Promise.resolve(this.handle));
    }
    async resolveFile(path, fileOptions, dirOptions) {
        const parts = FSDHFs.resolvePath(path);
        const filename = parts.pop();
        if (!filename)
            throw new Error("Cannot resolve empty path in file mode");
        const dir = await parts.reduce(async (handle, name) => (await handle).getDirectoryHandle(name, dirOptions), Promise.resolve(this.handle));
