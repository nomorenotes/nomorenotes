...
const DIGITSUTOL = Object.fromEntries(
  Array.prototype.map.call(DIGITSU, (u, i) => [u, DIGITSL[i]])
)
...
const invalidate = () => {
  Array(localStorage.length)
    .fill()
    .map((_v, i) => localStorage.key(i))
    .filter((k) => k?.startsWith("PWFUN_"))
    .forEach((k) => localStorage.removeItem(k))
  location.reload()
}
const totalTarget = +(localStorage.PWFUN_totalTarget ??=
  Math.round(Math.random() * 16) + 4)
const lowerTarget = +(localStorage.PWFUN_lowerTarget ??= Math.round(
  Math.random() * totalTarget
))
...
const ld = (localStorage.PWFUN_ld ??=
  "xc" + String.fromCharCode(96 + Math.floor(Math.random() * 26)))
const ul$ = fetch(new URL("users.txt", location.href))
  .then((r) => r.text())
  .then((t) =>
    t
      .trim()
      .split("\n")
      .map("".trim.call.bind("".trim))
...
if (username.value === "" && localStorage.PWFUN_NMNname)
  username.value = localStorage.PWFUN_NMNname
...
form.onsubmit = async (e) => {
  ...
  e(puu.length < st)`Password must have at least ${
    puu.length + 1
  } characters (${p} has ${puu.length})`
  e(puu.length > st)`Password must have at most ${
    puu.length - 1
  } characters (${p} has ${puu.length})`
  // bucket password
  ...
  for (let char of p) {
    ...
    e(
      char == last
    )`Password may not contain repeated characters (${char}${last} appears twice in ${p}, ${char} ${
      char === last ? "===" : "!=="
    } ${last})`
    console.log(last)
    last = char
...
form.onsubmit = (async (orig, ev, ...args) => {
  ...
  try {
    ...
    if (username.value === target) succeed(target)
    else
      eify(ev)()`Your password (${p}) is in use by ${target}, try another one.`
  } catch (e) {
...
form.onsubmit = (async (orig, ev, ...args) => {
  ...
  try {
    submitting = username.disabled = password.disabled = submit.disabled = true
    err.innerHTML = '<div class=\\"border-demo-05\\"></div>'
    return orig(ev, ...args)
  } finally {
    submitting = username.disabled = password.disabled = submit.disabled = false
  }
}).bind(null, form.onsubmit)
...
function checkBucketed(e, p, buckets, ord) {
  e(buckets.Dl < 1)`Password must contain at least 1 number (${p} has 0)`
  e(
    buckets.Lu == 0
  )`Password must contain at least 1 uppercase letter (${p} has 0)`
  e(buckets.Lu > 1)`Password must contain at most ${
    buckets.Lu - 1
  } uppercase letter (${p} has ${buckets.Lu})`
  e(buckets.Dl < 2)`Password must contain at least 2 numbers (${p} has 1)`
  e(
    buckets.Du < 1
  )`Password must contain at least 1 uppercase number (${p} has 0)`
  e(buckets.Dl > 2)`Password must contain at most ${
    buckets.Dl - 1
  } numbers (${p} has ${buckets.Dl})`
  e(
    buckets.Du < 2
  )`Password must contain at least 2 uppercase numbers (${p} has 1)`
  e(buckets.Du > 2)`Password must contain at most ${
    buckets.Du - 1
  } numbers (${p} has ${buckets.Du})`
  e(
    buckets.Sl == 0 && buckets.Su == 0
  ...
  e(buckets.Sl > 1)`Password must contain at most ${
    buckets.Sl - 1
  } lowercase symbol (${p} has ${buckets.Sl})`
  e(buckets.Su == 0)`Password must contain at least 1 symbol (${p} has 0)`
  e(buckets.Su > 1)`Password must contain at most ${
    buckets.Su - 1
  } uppercase symbol (${p} has ${buckets.Su})`
