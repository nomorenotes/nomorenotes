;(() => {
  ...
  const redir = (loc = location.href, cb = () => {}) =>
    cb(setTimeout(() => (location = loc), 5000))
  write("Processing")
    ...
    if (session === localStorage.session) {
      if (changeName) {
        ...
        const newName = prompt(
          `The admin unbanning you requires you to change your name. Please change your name.`,
          oldName
        )
        if (newName) {
          if (oldName === newName) {
      ...
      if (decrease) {
        ...
        const secs = time % 60
        write(
          `Your ban time has been decreased by ${mins}:${secs
            .toString()
            .padStart(2, 0)}. You will now be unbanned ${Date(
            +localStorage.banExpiry2
          )}. Returning to banned screen in 5 seconds.`
        )
        redir("/banned")
      } else {
        delete localStorage.banExpiry2
        write(
          `You have been completely unbanned. Returning to chat in 5 seconds.`
        )
        redir("/chat")
      }
    } else {
      write(
        "This URL is for a different user. Please send it to the correct user, or ask an admin if it was for you."
      )
    }
  }
})()
