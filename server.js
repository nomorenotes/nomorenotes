...
app.get("/getfile/:anything", (req, res) => {
  ...
  switch (req.params.anything) {
    case "greetings":
      res.redirect(
        "https://docs.google.com/presentation/d/1fr2I4QLh6i9WxafZuw6vOyh84BAW1tAgqCgLDYpE35w/edit#slide=id.ge50e878efe_0_40"
      );
      break;
    case "logicdnd":
      res.redirect(
        "https://docs.google.com/drawings/d/1NsU67czLjI1VBW3D020UtimUjPahgtRLGjDv8TFQxGE/edit"
      );
      break;
    case "chloetest":
      res.redirect("https://www.123test.com/iq-test/id=XHKTV7OU4OEJ&version=");
      break;
    case "ρw.txt": {
      res.send(
        `<style>
        ...
        .pw:active {
          ...
          Object.entries(data.creds)
            .map(
              ([u, p]) => `
        <h2>${u}</h2>
        <ul>
          ${p
            .map((ρ) => `<li><span class="pw" tabindex="-1">${ρ}</span></li>`)
            .join("\n")}
        </ul>
      `
            )
...
const map_range = (exports.map_range = function map_range(
  value,
  low1,
  high1,
  low2,
...
app.get("/oidc.json", requiresAuth(), ({ oidc }, res) => {
  Promise.resolve(Object.getPrototypeOf(oidc))
    .tap.then((p) => Object.getOwnPropertyNames(p))
    .tap.then((names) => names.map((name) => [name, oidc[name]]))
    .tap.then(Object.fromEntries)
    .tap.then((obj) => inspect(obj, INSPECTARGS))
    .tap.then((text) => res.send(text));
});
...
function getUserColor(oidc) {
  ...
  return [0xcafe0000, 0xcafe1111, 0xcafe2222].map((n) =>
    getUserRandom(oidc, n)
  );
}
...
loadadmin: if (true) {
  try {
    ...
    logger(
      "@socket.io/admin-ui is not installed.",
      "Admin UI will be unavailable.",
      logger.WARN
    );
    break loadadmin;
  }
